    ...
    def create_move(self, cr, uid, ids, context=None):
        ...
        for line in self.browse(cr, uid, ids, context=context):
            ...
            move_line_obj.create(cr, uid, {
                ...
                'debit': 0.0 if float_compare(amount, 0.0, precision_digits=precision) > 0 else -amount,
                'credit': amount if float_compare(amount, 0.0, precision_digits=precision) > 0 else 0.0,
                'period_id': period_ids and period_ids[0] or False,
            ...
            move_line_obj.create(cr, uid, {
                ...
                'credit': 0.0 if float_compare(amount, 0.0, precision_digits=precision) > 0 else -amount,
                'debit': amount if float_compare(amount, 0.0, precision_digits=precision) > 0 else 0.0,
                'period_id': period_ids and period_ids[0] or False,
                'journal_id': journal_id,
                'partner_id': partner_id,
                'currency_id': company_currency != current_currency and  current_currency or False,
